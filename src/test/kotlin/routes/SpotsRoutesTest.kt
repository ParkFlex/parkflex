...
class SpotsRoutesTest {
    ...
    fun `test get spots availability success`() = testApplication {
        ...
        val response = client.get("/api/spots?start=$start&end=$end")

        assertEquals(HttpStatusCode.OK, response.status)
        val body = response.body<SpotsModel>()

        assertTrue(body.spots.first { it.displayOrder == 1 }.occupied)
        assertFalse(body.spots.first { it.displayOrder == 2 }.occupied)
    }
    ...
    fun `test get spots availability considering break duration`() = testApplication {
        ...
        assertTrue(body.spots[0].occupied, "Spot should be occupied due to break duration")
    }
    ...
    fun `test get spots invalid date format`() = testApplication {
        ...
        val response = client.get("/api/spots?start=2024-10-10&end=nie-data")
    ...
    fun `test get spots end date before start date`() = testApplication {
        ...
        val start = "2024-10-10T15:00:00"
        val end = "2024-10-10T14:00:00"
