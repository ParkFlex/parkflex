...
class SpotRoutesTest {
    ...
    fun `test get spot success with reservations`() = testApplication {
        ...
        val spotId = runDB {
            ...
            val user = UserEntity.new(id = 2) {
                fullName = "Test User"; mail = "test@parkflex.pl"; hash = "hash"; plate = "PO-TEST"; role = "user"
            }
        ...
        val response = client.get("/api/spot?spot_id=$spotId")
    ...
    fun `test get spot spot does not exist`() = testApplication {
        ...
        runDB { SchemaUtils.create(SpotTable) }

        val response = client.get("/api/spot?spot_id=999")
    ...
    fun `test get spot invalid id format`() = testApplication {
        ...
        val response = client.get("/api/spot?spot_id=abc")
